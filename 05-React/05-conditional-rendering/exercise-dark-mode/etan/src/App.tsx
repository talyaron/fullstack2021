...
function App() {
  const [themeSelector, setThemeSelector] = useState<boolean>(true)
  ...
  // }
if(themeSelector) {
  var { primary, secondary } = darkTheme.palette;
} else{
  var{primary, secondary} = lightTheme.palette;
}
...
<Switch
color="secondary"
onChange={() => {setThemeSelector(!themeSelector)}}
/>
            }
            labelPlacement="top"
        ...
        <Typography variant="h1" color={primary.contrastText}>{`${themeSelector}`}</Typography>
              <TextField
