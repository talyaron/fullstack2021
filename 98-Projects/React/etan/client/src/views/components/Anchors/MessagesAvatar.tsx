...
function MessagesAvatar(props: MessagesAvatarProps) {
    const {AvatarStyling, setPopoverButton, setMessagesAnchor} = props;
    function handleOpenNavPopover(ev: any) {
      const button = ev.currentTarget.firstChild;
      const icon = ev.currentTarget
      setPopoverButton(icon);
      setMessagesAnchor(button);
  ...
  return (
    <motion.div    >
      <Avatar style={AvatarStyling} onClick={(ev)=>handleOpenNavPopover(ev)}>
        <FontAwesomeIcon size="xs" icon={["fas", "comment-dots"]} />
      </Avatar>
