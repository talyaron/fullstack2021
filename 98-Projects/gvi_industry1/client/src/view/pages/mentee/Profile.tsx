import React from 'react'
import './style/profile.scss';
import motion from 'framer-motion';

const userDetails = {
    name:'Ido gino',
    email:'lalala@gmail.com',
    descriotion:'lalalalalala',
    img:'data:image/png;base64,',
    country: 'Israel',
    lastEntry: '09.01.2021',
    companies:['Apple ,facebook'],
    linkedInProfile: 'https://linkdin.com',
    phone: 972547554056,
    type: 'Mentee',
    fieldsOfKnowledge: ['History', 'Math' , 'Finance'],
    sectors:'eduction',
    profession:'Finance'
}

export function Profile () {
  return (
    <div className='profile'>
        <div className="profile_profilePic" style={{backgroundImage:`url(${userDetails.img})`}}></div>
        <div className='profile_name'>
        {userDetails.name}
        </div>
        <div className='profile_country'>
        Countrey : {userDetails.country}
        </div>
        <div className='profile_email'>
        email : {userDetails.email}
        </div>
        <div className='profile_phone'>
        contact number : {userDetails.phone}
        </div>
        <div className="profile_profession">
            {userDetails.profession}
        </div>
        <div className="profile_lastEntry">
            last entry : {userDetails.lastEntry}
        </div>
        <div className="profile_linkedInProfile">
            Link to social media profile : {userDetails.linkedInProfile}
        </div>
        <div className="profile_fieldsOfKnowledge">
            <h1 className='profile_fieldsOfKnowledge-head'>Fields of knowledge :</h1>
            {userDetails.fieldsOfKnowledge.map(field => {
                return (
                    <div className='profile_fieldsOfKnowledge-field'>{field}</div>
                )
            })}
        </div>
        
    
    </div>
  )
}
export default Profile
