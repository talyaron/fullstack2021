//@ts-ignore
import LinkedInIcon from "@mui/icons-material/LinkedIn";
//@ts-ignore
import "./style/cardReq.scss";
import Popup from 'reactjs-popup';
  import 'reactjs-popup/dist/index.css';
  import { useNavigate } from "react-router-dom";


interface CardProps {
  RequestUsers: any;
}

const Card = (props: CardProps) => {
  const { RequestUsers } = props;
  const navigate = useNavigate();

  return (
    <>
      {RequestUsers.map((item: any, i: any) =>
        <div className="card" key={i}>
         
            <Popup trigger={<img id='envelope' src={`${"https://mpng.subpng.com/20180606/xbk/kisspng-email-clip-art-open-envelope-5b1885b6f1a5f1.8092976715283337509898.jpg"}`} alt="" />} position="right center">
                          
              <div>Hi! I am interested in learning more about your venture and would like to see if we an collaborate</div></Popup>
              
          <div className="card__photo">
            <img src={item.image ? `${item.image}` : 'data:image/jpeg;base64,'} />
          </div>

          <div className="card__center">
            <p>{item['name']['first']} {item['name']['last']}</p>

            <div className="card__flex">
              <img src={item.country ? `${item.country}` : 'https://flagsweb.com/Flag_Colouring_Page/WEBP/Germany_Flag_Colouring_Page.webp' }/>
              <a href={item.linkedInProfile ? `${item.linkedInProfile}` : 'https://www.linkedin.com/'}>
                <LinkedInIcon className="card__flex__linkdIn" style={{ fontSize: "30px" }} ></LinkedInIcon>
              </a>
            </div>
            <p className="card__center__company">{item.fieldsOfKnowledge}</p>
            <p className="card__center__profession">{item.sector}</p>
        
          </div>
         <div className='request-card-buttons-flex'>
                {/* <button onClick={()=> window.open("someLink", "_blank")}>text</button> */}
                <Popup trigger={<button className='request-card-flex__mismatch' >Mismatch</button>} position="right center">
                  <div>
                    <form>
                        <label>
                            Select a reason for rejecting the request:
                              <input type="text" name="name" /><br></br>
                      </label>
                      <label>       
                            You can add a short message(optional):
                              <input type="text" name="name" placeholder='(Up to 50 characters)' />
                      </label>
                                <input type="submit" value="Submit" />
                    </form>                           
                  </div>
                </Popup>  
               

                <button className="match"onClick={()=>{navigate('/mainPage/chat')}}>Start Mentoring</button>
                
                

              </div>

        </div>
        
        
      )
      }
      
    </>
  );
};

export default Card;
