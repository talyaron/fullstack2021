...
var Adidas = {
  ...
  addItem: function addItem(title, price, gender, category, pictureFront, pictureBack, color, description, shoeSize) {
    var id = uid();
    this.getData();
    this.products.push({
      id: id,
  ...
  handleOwner: function handleOwner(list, domElement) {
    console.log(list);
    var html = "";
    this.products.forEach(function (product) {
  ...
  updateItem: function updateItem(id, newTitle, newPrice, newGender, newCategory, newPictureFront, newPictureBack, newColor, newDescription, newShoeSize) {
  ...
  renderAllData: function renderAllData(domElement) {
    this.render(this.products, domElement);
  },
  filterItems: function filterItems(highPrice, lowPrice) {
    return this.products.filter(function (item) {
      return item.price >= lowPrice && item.price <= highPrice;
    });
  },
  shoeSizeFilter: function shoeSizeFilter(size) {
    return this.products.filter(function (item) {
      return item.size === size;
    });
  },
  sortItemsAsc: function sortItemsAsc() {
    this.products.sort(function (x, y) {
      return y.price - x.price;
    });
  },
  sortItemsDsc: function sortItemsDsc() {
    this.products.sort(function (x, y) {
      return x.price - y.price;
    });
  },
  sortByGender: function sortByGender(item) {
    return this.products.filter(function (element) {
      return element.gender === item;
    });
  },
  sortByColor: function sortByColor(color) {
    return this.products.filter(function (element) {
      return element.color === color;
    });
  },
  sortByType: function sortByType(category) {
    return this.products.filter(function (element) {
      return element.category === category;
    });
  },
  renderFilter: function renderFilter(filterd, domElement) {
    console.log(filterd);
...
function handleType(ev) {
  ...
  if (type === "Sneakers") {
    console.log(type); // Adidas.sortByType(type)
    //      Adidas.renderAllData(root);
...
function handleColor(ev) {
  ...
  ev.preventDefault();
  var root = document.getElementById("rootCustomer"); // if(type === " ")
...
function handleShoeSize(ev) {
  ...
  ev.preventDefault();
  var root = document.getElementById("rootCustomer");
  Adidas.renderFilter(Adidas.shoeSizeFilter(size), root); // if(type === " ")
...
function handleGetData(page) {
  try {
    // console.log(page);
    Adidas.getData(); // console.log(Adidas);
