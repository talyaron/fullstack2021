...
function getMoviesByRank(Movies){
    return Movies.sort((x, y) => x.ranking - y.ranking)
}
const movieRanks = getMoviesByRank(Movies)
console.log(movieRanks)

function getMoviesByYear(Movies){
    return Movies.sort((x, y) => x.yearCreated - y.yearCreated)
}
const movieYear = getMoviesByYear(Movies)
