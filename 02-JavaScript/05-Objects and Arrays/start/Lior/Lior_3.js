...
array = [Lior.Name, Victoria.Name, Ilan.Name];
...
all_friends=["Barak", "Yefet", "Igor", "Eyal", "Or", "Shimon","Simon", "Yefet", "Igor","Natasha", "Yulia"]


//Q5 - I don't undetand why it's not running properly.

    for(i=0;i<array.length; i++)
    {
        for(j=0;j<array[i].Number_of_friends; j++)
        {   
            console.log(`${array[i].Name} has the following 
            
            freinds: ${all_friends[j]},`)
        }
    }
    ...
    function family(Yage,YName,YProffestion,YDirthOfDate,YNumber_of_friends)
    {
         YName=
        {
            age:Yage,
            Name: `${YName}`,
            Proffestion: `${YProffestion}`,
            DirthOfDate: `${YDirthOfDate}`,
