...
app.get("/get-games-by-price", (req, res) => {
  try {
    ...
    if (maxPrice) {
      ...
      const filteredGames: Array<Game> = games.filter(
...
app.delete("/delete-game", (req, res) => {
  try {
    ...
    if (id) {
      ...
      if (index === -1) throw new Error("Didnt find games with such an id");
...
app.patch("/update-game", (req, res) => {
  try {
    ...
    if (!id || !title) throw new Error("No id or title in body");
    //update
    //find
    const index = games.findIndex((game) => game.id === id);
    //update
