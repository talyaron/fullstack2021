...
app.get("/get-all-products", (req, res) => {
    ...
    setTimeout(() => {
        if(category){
          const filterUsers =  Adidas.products.filter(
   ...
   const genderFilter = Adidas.products.filter(
      (element) => element.gender === Gender
    );
    res.send(genderFilter);
  }
  else if(Color){
    const colorFilter = Adidas.products.filter(
      (element) => element.color === Color
    );
    res.send(colorFilter);
  }
  else if(ShoeSize){
  const shoeSizeFilter =  Adidas.products.filter(
      (item) => item.shoeSize === ShoeSize
    );
