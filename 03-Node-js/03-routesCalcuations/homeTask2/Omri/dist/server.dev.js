...
app.get('/getArticels', function (req, res) {
  var searchTerm = req.query.searchTerm;
  var filterdArticels = filterdArtiels(searchTerm);
...
function filterdArtiels(searchTerm) {
  ...
  try {
    ...
    if (searchTerm.length > 0) {
      filterdArticels = articels.filter(function (article) {
        return article.title.match(regex) || article.subject.match(regex) || article.paragraph.match(regex);
      });
